...
function createWindow() {
    ...
    // Renderer-requested hard reload (ignore cache).
    // This is used by the in-app "Refresh UI" recovery button.
    try {
        ipcMain.removeAllListeners('bf6-reload-ignoring-cache');
        ipcMain.on('bf6-reload-ignoring-cache', () => {
            try {
                if (!mainWindow || mainWindow.isDestroyed()) return;
                if (mainWindow.webContents && typeof mainWindow.webContents.reloadIgnoringCache === 'function') {
                    mainWindow.webContents.reloadIgnoringCache();
                    return;
                }
            } catch (e) {
                console.warn('[BF6] reloadIgnoringCache failed:', e);
            }
            try {
                if (!mainWindow || mainWindow.isDestroyed()) return;
